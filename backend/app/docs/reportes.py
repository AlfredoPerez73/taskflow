...
@enrutador.get("/proyectos/{proyecto_id}/auditoria", response_model=List[RespuestaEntradaAuditoria],
    summary="Log de auditoría del proyecto",
    description="Devuelve los últimos **100 registros** de auditoría del proyecto ordenados por fecha descendente. Incluye creación, modificación, movimiento y eliminación de tareas.")
async def auditoria_proyecto(proyecto_id: str, _: dict = Depends(obtener_usuario_actual)):
    return await servicio_reporte.obtener_auditoria_proyecto(proyecto_id)
